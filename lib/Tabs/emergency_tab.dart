...
class _EmergencyTabState extends State<EmergencyTab> {
  ...
  Future<void> loadPosts({bool refresh = true}) async {
    await setUserData();
    String postEmergencyRaw = json.encode(await request.API.getPostEmergency(user['uid'], refresh: refresh));
...
class _CardWidgetState extends State<CardWidget> {
  Color mainColor = Colors.red.shade500;
                      ...
                      TextButton.icon(
                        onPressed: () async {
                          ...
                          if(response["status"] == true) {
                            await request.API.getPostEmergency(widget.username, refresh: true);
                            if (kDebugMode) {
